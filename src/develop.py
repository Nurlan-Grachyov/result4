...
class OperationsWithVacancies(GetVacancies):
    ...
    def filtered_vacancies(self):
        for vacancy in self.vacancies:
            if vacancy.get("salary") is None or vacancy["salary"].get("to") is None or vacancy["salary"].get("from") is None or vacancy in self.sorted_vacancies:
                continue
            if (
...
class DataFile(OperationsWithVacancies, AbstractFile):  # Remove OperationsWithVacancies from inheritance
    ...
    def save_data(self):
        with open(self.file, "w", encoding='utf-8') as file:
            json.dump(self.comparison_pay(), file, ensure_ascii=False)
...
if __name__ == '__main__':
    ...
    # sorted_data.filtered_vacancies()
    ...
    save_data = data_to_file.delete_data()
    print(save_data)